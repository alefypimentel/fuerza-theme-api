...
2. **Adição de Traduções**: O método `add_translation_data` adiciona todas as traduções disponíveis
3. **Formatação ACF**: Os campos ACF são formatados incluindo suas traduções
4. **Filtros de Tradução**: Os filtros `fuerza_api_format_post` são aplicados para compatibilidade
...
✅ Traduções ACF encontradas: EN (2 campos), ES (2 campos)
...
O sistema agora retorna todas as traduções disponíveis independente do idioma solicitado, exatamente como especificado no exemplo de saída desejado.
